        ...
        for split, meta in splits:
            ...
            for file in tqdm(meta):
                ...
                if len(vad_segments) > 0:
                    ...
                    new_meta[split].append(new_file)

            print("discarded", check_index - kept)

            # Write out updated metadata.
            with open(self.out_dir + split + ".json", 'w') as outfile:
                json.dump(new_meta, outfile)
...
def parse_args():
    ...
    parser.add_argument('--out_dir', type=str, default='data_speech_isolated_silero/', help='Directory to save datasets to')
